...
async def _solve(sitekey: str, siteurl: str, timeout: int, action: Optional[str] = None) -> str:
    ...
    try:
        ...
        # NOTE: sitekey/action are our own controlled values, but json.dumps()
        # is used to embed them safely as JS string/null literals regardless.
        render_opts = {"sitekey": sitekey}
        if action:
            render_opts["action"] = action
        render_opts_json = json.dumps(render_opts)
        await page.evaluate(f"""
            (() => {{
                ...
                window._tsLoad = function () {{
                    const opts = {render_opts_json};
                    opts.callback = function(token) {{ window._tsToken = token; }};
                    turnstile.render('#_ts_box', opts);
                }};
        ...
        async def get_cf_iframe_rect() -> Optional[dict]:
            # NOTE: Cloudflare renders the Turnstile iframe inside a *closed*
            # shadow root, so it is unreachable via document.querySelectorAll('iframe').
            # We instead measure our own wrapper div (#_ts_box, plain light DOM),
            # which Turnstile renders into and sizes to fit the widget (~300x65).
...
def solve(sitekey: str, siteurl: str, timeout: int = 45, action: Optional[str] = None) -> str:
    import warnings
